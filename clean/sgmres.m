% The Simpler GMRES algorithm modified to compute and return various statistics about the convergence assuming the exact solution is known
% INPUT:
...
%   Z_condition_numbers The condition number of Z = V in each iteration
...
function [x,residual_norms,backward_error,forward_error,true_residual,updated_residual,Z_condition_numbers,U_condition_numbers] = sgmres( A, b, tol, true_x, x0, iter_count )
  ...
  while residual_norms(n) > tol && n <= iter_count
    ...
    residual_norms(n) = norm(r);
    Z(:,n) = V(:,n-1);
    x = x0 + Z(:,1:n-1)* backsub(U(1:n-1,1:n-1),alpha(1:n-1));
